  ...
  return (
    <div>
      <h2 id="employee-heading" data-cy="EmployeeHeading">
        <Translate contentKey="skillTestApp.employee.home.title">Employees</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" variant="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="skillTestApp.employee.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/employee/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="skillTestApp.employee.home.createLabel">Create new Employee</Translate>
          </Link>
        </div>
      </h2>
        ...
        {employeeList?.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="skillTestApp.employee.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('nom')}>
                  <Translate contentKey="skillTestApp.employee.nom">Nom</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('nom')} />
                </th>
                ...
                <th>
                  <Translate contentKey="skillTestApp.employee.poste">Poste</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {employeeList.map(employee => (
                <tr key={`entity-${employee.id}`} data-cy="entityTable">
                  ...
                  <td>
                    {employee.dateEmbauche ? <TextFormat type="date" value={employee.dateEmbauche} format={APP_LOCAL_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    <Translate contentKey={`skillTestApp.Role.${employee.role}`} />
                  </td>
                  <td>{employee.poste ? <Link to={`/poste/${employee.poste.id}`}>{employee.poste.intitule}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="skillTestApp.employee.home.notFound">No Employees found</Translate>
            </div>
          )
        )}
      </div>
