...
import com.mycompany.skilltest.service.ReponseQueryService;
...
/**
 * REST controller for managing {@link com.mycompany.skilltest.domain.Reponse}.
 */
@RestController
@RequestMapping("/api/reponses")
@Secured({ AuthoritiesConstants.ADMIN, AuthoritiesConstants.MANAGER, AuthoritiesConstants.EXPERT, AuthoritiesConstants.EMPLOYEE })
public class ReponseResource {

    private static final Logger LOG = LoggerFactory.getLogger(ReponseResource.class);
    ...
    private final ReponseQueryService reponseQueryService;

    public ReponseResource(ReponseService reponseService, ReponseRepository reponseRepository, ReponseQueryService reponseQueryService) {
        this.reponseService = reponseService;
        this.reponseRepository = reponseRepository;
        this.reponseQueryService = reponseQueryService;
    }
    ...
    public ResponseEntity<ReponseDTO> createReponse(@Valid @RequestBody ReponseDTO reponseDTO) throws URISyntaxException {
        ...
        reponseDTO = reponseService.save(reponseDTO);
        return ResponseEntity.created(new URI("/api/reponses/" + reponseDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, reponseDTO.getId().toString()))
            .body(reponseDTO);
    }
    ...
    public ResponseEntity<ReponseDTO> updateReponse(
        ...
        reponseDTO = reponseService.update(reponseDTO);
    ...
    public ResponseEntity<List<ReponseDTO>> getAllReponses(
        ReponseCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get Reponses by criteria: {}", criteria);

        Page<ReponseDTO> page = reponseQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /reponses/count} : count all the reponses.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countReponses(ReponseCriteria criteria) {
        LOG.debug("REST request to count Reponses by criteria: {}", criteria);
        return ResponseEntity.ok().body(reponseQueryService.countByCriteria(criteria));
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<ReponseDTO> getReponse(@PathVariable("id") Long id) {
